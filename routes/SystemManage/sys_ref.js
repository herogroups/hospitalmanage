const db = require('../../DbLib/SystemManage/Sys_Ref_DB')
const Ctrlbase = require('../CtrBase')
/**
 * 命令业务控制模块
 */
class sys_ref  extends Ctrlbase{
  /***
   * 进入查询索引控制页面
   */
  async Index(ctx, next) {
       
    await ctx.render('/SystemManage/Sys_Ref/Index', {});
  }
  /***
   * 进入添加界面
   */
  async AddIndex(ctx, next) {
    await ctx.render('/SystemManage/Sys_Ref/AddIndex', {});
  }
  /**
   * 进入修改界面
   *  {*} ctx 
  ...
  async UpdateIndex(ctx, next) {
    ...
    await ctx.render('/SystemManage/Sys_Ref/UpdateIndex', {
      'Model': result
    });
  }
  /**
  ...
  async Update(ctx, next) {
    ...
    try {
      ...
      result = await db.Update(body, body.REFID); //更新数据
