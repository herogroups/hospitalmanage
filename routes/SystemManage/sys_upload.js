const db = require('../../DbLib/SystemManage/Sys_Upload_DB')
const Ctrlbase = require('../CtrBase')
/**
 * 命令业务控制模块
 */
class sys_upload  extends Ctrlbase{
  /***
   * 进入查询索引控制页面
   */
  async Index(ctx, next) {
       
    await ctx.render('/SystemManage/Sys_Upload/Index', {});
  }
  /***
   * 进入添加界面
   */
  async AddIndex(ctx, next) {
    await ctx.render('/SystemManage/Sys_Upload/AddIndex', {});
  }
  /**
   * 进入修改界面
   *  {*} ctx 
  ...
  async UpdateIndex(ctx, next) {
    ...
    await ctx.render('/SystemManage/Sys_Upload/UpdateIndex', {
      'Model': result
    });
  }
  /**
  ...
  async Add(ctx, next) {
    ...
    try {
      ...
      if (result) {
        throw new Error(`插入失败,[${body.REMOTEIP}]已经存在`)
      }
  ...
  async Update(ctx, next) {
    ...
    try {
      ...
      if (result) {
        throw new Error(`修改失败,[${body.REMOTEIP}]已经存在`)
      }
      result = await db.Update(body, body.UPLOADID); //更新数据
